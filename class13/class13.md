...
- [Set up for DESeq](#set-up-for-deseq)
- [Run DESeq](#run-deseq)
...
Check correspondence of `metadata` and `counts` (i.e that the columns in
`counts` match the rows in the `metadata`)
...
```
    ...
    Info: Working in directory /Users/mariatavares/Desktop/BGGN213/bggn213_github/class13
...
rather than the R package of the same name(available from bioconductor)
